<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ModifyConsumerOrder.aspx.cs" Inherits="ModifyConsumerOrder" %>
		...
		<div id="content" class="layout grid-s160m0">
				<div class="col-main">
						<div class="main-wrap" style="margin-bottom:150px">
								...
								<form id="ConsumerOrderForm" runat="server" target="_self">
										<div align="center">
										<div align="left" style="margin-left:20px;">
											<asp:Label id="ShowNoOrderInfo" runat="server" Text="" CssClass="showNoProductInfo" />
										</div>
										<asp:DataList ID="ConsumerOrderList" runat="server" RepeatColumns="1" CssClass="bought-table">
												<ItemTemplate>
														<div class="mainOrder747075301432121  xcard" style="margin-bottom: 15px; margin-top: 15px;">
																<div class="sep-row" style="background-color: #afd9ff"></div>

																<div class="order-hd" style="background-color: #afd9ff">
																		<div class="first">
																				<div class="summary">
																						<span style="margin-left: 15px;">
																								<asp:CheckBox runat="server" ID="ConsumerOrderCheckBox" CssClass="J_Selector selector" />
																						</span>
																						<span>
																								<asp:Label runat="server" ID="ConsumerOrderDate" Text='<%#Eval("ConsumerOrderDateText") %>' CssClass="dealtime" />
																						</span>
																						<span class="number last-item">
																								<asp:Label runat="server" ID="ConsumerOrderID" Text='<%#Eval("ConsumerOrderIDText") %>' CssClass="someLabelStyle" />
																						</span>
																				</div>
																		</div>
																		<div class="column">
																				<span class="g-u" style="margin-left: 15px;">
																						<asp:Image runat="server" ImageUrl="~/img/decorate.png" />
																				</span>
																				<asp:HyperLink runat="server" ID="StoreLink" CssClass="shopname J_MakePoint" Target="_blank" Text='<%#Eval("StoreLinkText") %>' NavigateUrl='<%#Eval("StoreLinkURL") %>' />
																		</div>
																		<div class="last"></div>
																</div>

																<div class="order-bd last  order-last">
																		<div class="baobei">
																				<asp:HyperLink runat="server" ID="ProductImgLink" ImageUrl='<%#Eval("ProductImgURL") %>' CssClass="pic J_MakePoint" NavigateUrl='<%#Eval("ProductImgLinkURL") %>' Target="_blank" />
																				<div class="desc">
																						<p class="baobei-name">
																								<asp:HyperLink runat="server" ID="ProductLink" NavigateUrl='<%#Eval("ProductLinkURL") %>' Text='<%#Eval("ProductLinkText") %>' Target="_blank" CssClass="J_MakePoint" />
																						</p>
																				</div>
																		</div>

																		<div class="price someDivStyle" align="center">
																				<asp:Label runat="server" ID="ProductPrice" CssClass="special-num" Text='<%#Eval("ProductPriceText") %>' />
																		</div>

																		<div class="quantity someDivStyle" align="center">
																				<asp:Label runat="server" ID="ProductQuantity" CssClass="special-num" Text='<%#Eval("ProductQuantityText") %>' />
																		</div>

																		<div class="trade-status someDivStyle" align="center">
																				<asp:Label runat="server" ID="ConsumerOrderStatus" CssClass="J_MakePoint status" Text='<%#Eval("ConsumerOrderStatusText") %>' />
																		</div>

																		<div class="someDivStyle" align="center">
																				<!--这里回头加操作按钮-->
																				<asp:Label runat="server" Text="操作" />

    ...
    public class commentSync
    {
        ...
        public float product_score
        {
            ...
            set
            {
                if (!commentExist)
                    throw new System.Exception("NO_EXIST");
                myData.Rows[0]["product_score"] = value;
                pushData();
            }
        }
        ...
        public DataTable getAllComments(long iProductID)
        {
            ArrayList list = new ArrayList();
            SqlDataAdapter adp = new SqlDataAdapter("SELECT * FROM [tb_comment] WHERE product_id  = '" + iProductID + "'", myConnection);
            new SqlCommandBuilder(adp);
            DataTable table = new DataTable();
            adp.Fill(table);
